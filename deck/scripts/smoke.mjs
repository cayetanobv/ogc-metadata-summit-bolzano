...
const LOCALES = ['en'] // English-only deck; the CNG Japan locale switch is retired
...
async function walk(colorScheme, deckLocale) {
  ...
  try {
    for (const slide of SLIDES) {
      ...
      // English-only deck: the data-deck-locale attribute went away with the locale switch; only <html lang> is asserted.
      if (state.documentLang !== deckLocale) {
...
try {
  ...
  // await verifyBilingualPresenterSync()  // retired with the locale switch
} finally {
  await browser.close()
  local?.server.close()
}
