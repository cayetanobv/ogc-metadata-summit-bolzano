...
## Structure
- `slides.md` sets the order; one Markdown file per slide in `slides/`. Slides `a01`–`a15` are the talk
...
- `deck.config.mjs`: Pages base (`/ogc-metadata-summit-bolzano/`), main-slide count (15), click budget, export paths (`../export/`).
...
Visual QA: serve `dist/` under `/ogc-metadata-summit-bolzano/` and run `node scripts/render-slides.mjs <baseUrl> <outdir> 15`.
