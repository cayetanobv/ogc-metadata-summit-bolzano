...
## Structure
- `slides.md` sets the order; one Markdown file per slide in `slides/`. Slides `a01`–`a17` are the talk (17 slides, the
  last one is the Questions slide); `x01`–`x03` are the appendix (why "Portolan", ISO 19115 / INSPIRE, the ISO JSON example).
...
- `deck.config.mjs`: Pages base (`/ogc-metadata-summit-bolzano/`), main-slide count (17), click budget, export paths (`../export/`).
...
- Talk order: what Portolan is → why (five goals) → building blocks → lifecycle → what it adds → story 1 → where the metadata
...
Visual QA: serve `dist/` under `/ogc-metadata-summit-bolzano/` and run `node scripts/render-slides.mjs <baseUrl> <outdir> 17`.
