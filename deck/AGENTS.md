...
## Structure
- `slides.md` sets the order; one Markdown file per slide in `slides/`. Slides `a01`–`a17` are the talk (16 slides; `16-honest-work.md` moved to the backups); `b12`, `b15`, `b06`, `b08` follow as Q&A backups.
...
- `deck.config.mjs`: Pages base (`/ogc-metadata-summit-bolzano/`), main-slide count (16), click budget, export paths (`../export/`).
...
Visual QA: serve `dist/` under `/ogc-metadata-summit-bolzano/` and run `node scripts/render-slides.mjs <baseUrl> <outdir> 16`.
