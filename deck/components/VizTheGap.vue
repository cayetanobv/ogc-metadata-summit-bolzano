...
<template>
  <div class="stage">
    <svg class="canvas" viewBox="0 0 800 460" role="group"
         :aria-label="tr('STAC 1.1.0 supplies the catalog, collection, item, asset, metadata, and link model. Portolan version 0.2.0 profiles how that static STAC publication is structured, documented, traced, optimized, hosted, and visualized. The catalog declares the versioned Portolan schema URI, and the specification remains pre-1.0.', 'STAC 1.1.0はCatalog、Collection、Item、Asset、メタデータ、リンクのモデルを提供します。Portolan v0.2.0は、その静的STAC公開物の構造、文書、来歴、最適化、ホスティング、可視化をプロファイルします。カタログはバージョン付きPortolanスキーマURIを宣言し、仕様は現在もpre-1.0です。')">
      <defs>
      ...
      <text x="400" y="27" text-anchor="middle" class="kicker">STAC 1.1.0 {{ tr('BASELINE', '基盤') }} + PORTOLAN v0.2.0 {{ tr('PROFILE', 'プロファイル') }}</text>
      ...
      <g class="portolan" :class="{ on: s >= 2 }">
        <a href="https://github.com/portolan-sdi/portolan-spec/releases/tag/v0.2.0"
           target="_blank" rel="noopener noreferrer" class="svg-source-link"
           :aria-label="tr('Open Portolan specification release 0.2.0 in a new tab', 'Portolan仕様リリース0.2.0を新しいタブで開く')" @click.stop>
          ...
          <text x="602" y="117" class="portolan-version">v0.2.0 · pre-1.0</text>
